...
{
    return (sprite_object == NULL) ? 0 : sprite_object->ty;
}
...
{
    return (sprite_object == NULL) ? 0 : sprite_object->tscale;
}
...
void sprite_object_set_tscale(SpriteObject* sprite_object, FIXED tscale)
{
    if (sprite_object == NULL)
        return;
    sprite_object->tscale = tscale;
}

FIXED sprite_object_get_trotation(const SpriteObject* sprite_object)
{
    return (sprite_object == NULL) ? 0 : sprite_object->trotation;
}
